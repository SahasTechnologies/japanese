...
import LISTENING_SCRIPTS from '../data/listening-n4.json' with { type: 'json' };
...
export function listeningQs4(n) {
...
export function renderListening4() {
...
`;
  ...
  const onDone = (s, t) => updateBest('listening4', s, t);

  document.getElementById('lq-btn').onclick = () =>
    runFullQuiz(listeningQs4(20), { onDone, onExit: renderListening4, backLabel: '← Listening' });

  document.getElementById('lmany-btn').onclick = () =>
    runFullQuiz(listeningQs4(50), { onDone, onExit: renderListening4, backLabel: '← Listening' });
  ...
  document.getElementById('lscript-btn').onclick = () => {
    ...
    runFullQuiz(qs, { onDone, rate: 0.9, onExit: renderListening4, backLabel: '← Listening' });
  };

  document.getElementById('lslow-btn').onclick = () =>
    runFullQuiz(listeningQs4(20), { rate: 0.7, onDone, onExit: renderListening4, backLabel: '← Listening' });
}
