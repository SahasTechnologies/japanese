import VOCAB from '../data/vocab-n4.json' with { type: 'json' };
...
export function exampleSentences4(expr, reading, meaning) {
...
function displayHtml4(w) {
  return formatVocabWord(w[0], w[1], null, 'n4');
}

export function vocabQs4(words, n) {
...
export function renderVocab4() {
  const main = document.getElementById('main');
  const P = getState();
  const learnedCount = P.vocabLearned4.length;
  const mode = P.vocabKanjiMode || 'learned';

  main.innerHTML = `
    <div class="sec-title">Vocabulary</div>
    <div class="sec-sub">${ALLVOCAB.length} JLPT N4 words · ${learnedCount} marked learned
  ...
  Object.keys(VOCAB).forEach(cat => {
    ...
    const n = VOCAB[cat].filter(w => P.vocabLearned4.includes(w[0])).length;
    b.textContent = `${cat} (${VOCAB[cat].length}${n ? ` · ${n}✓` : ''})`;
    b.onclick = () => { vcat = cat; selectedWord = null; renderVocab4(); };
    vct.appendChild(b);
  });

  // SRS review
  const srsCards = vocabSrsCards4();
  ...
  VOCAB[vcat].forEach(w => {
    const learned = P.vocabLearned4.includes(w[0]);
    ...
    let detail = '';
    if (isOpen) {
      const exs = exampleSentences4(w[0], w[1], w[2]);
      detail = `
        <div class="v-detail">
          <div class="v-reading-line">
            <span class="v-full">${formatVocabWord(w[0], w[1], null, 'n4')}</span>
    ...
    card.innerHTML = `
      <div class="vcard-top">
        <span class="w">${displayHtml4(w)}</span>
  ...
  const onDone = (s, t) => updateBest('vocab4', s, t);
  document.getElementById('vq-btn').onclick = () =>
    runFullQuiz(vocabQs4(VOCAB[vcat], Math.min(12, VOCAB[vcat].length)), { onDone, onExit: renderVocab4, backLabel: '← Vocabulary' });
  document.getElementById('vqa-btn').onclick = () =>
    runFullQuiz(vocabQs4(ALLVOCAB, 20), { onDone, onExit: renderVocab4, backLabel: '← Vocabulary' });
  const vql = document.getElementById('vql-btn');
  if (vql && !vql.disabled) {
    vql.onclick = () => {
      const learnedWords = ALLVOCAB.filter(w => P.vocabLearned4.includes(w[0]));
      runFullQuiz(vocabQs4(learnedWords, Math.min(15, learnedWords.length)), { onDone, onExit: renderVocab4, backLabel: '← Vocabulary' });
    };
  }
}
